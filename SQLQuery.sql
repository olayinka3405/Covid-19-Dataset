...
SELECT TOP 20 CD.location AS Countries, MAX(CAST(CV.[people_fully_vaccinated] AS NUMERIC)) AS 'Population Vaccinated', SUM(CD.[new_cases]) AS 'Total Case', (SUM(CD.[new_cases]) / SUM(CAST(CV.[people_fully_vaccinated] AS NUMERIC)))*100 AS 'Infection Rate'
